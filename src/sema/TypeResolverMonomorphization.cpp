...
public:
    explicit MonomorphizationCloner(const std::unordered_map<std::string, TypePtr>& typeBindings)
...
private:
    ...
    template <typename T>
    static std::unique_ptr<T> located(std::unique_ptr<T> clone, const ASTNode* source) {
        if (clone && source) {
    ...
    void fail(const char* category) {
        if (failureMessage.empty())
            failureMessage =
                std::string("unsupported ") + category + " AST node in generic function body";
    }
    ...
    std::unique_ptr<TypeAST> cloneType(const TypeAST* src) {
        ...
        if (auto* type = dynamic_cast<const NamedTypeAST*>(src)) {
            ...
            if (auto bound = bindings.find(type->name); bound != bindings.end()) {
        ...
        if (auto* type = dynamic_cast<const RefTypeAST*>(src)) {
            ...
            return located(std::make_unique<RefTypeAST>(std::move(inner), type->isMutable), src);
        }
        if (auto* type = dynamic_cast<const LinearTypeAST*>(src)) {
            auto inner = cloneType(type->inner.get());
            if (!inner) return nullptr;
            return located(std::make_unique<LinearTypeAST>(std::move(inner)), src);
        }
        if (auto* type = dynamic_cast<const AffineTypeAST*>(src)) {
            auto inner = cloneType(type->inner.get());
            if (!inner) return nullptr;
            return located(std::make_unique<AffineTypeAST>(std::move(inner)), src);
        }
    ...
    std::unique_ptr<Expr> cloneExpr(const Expr* src) {
        ...
        if (auto* expr = dynamic_cast<const StringLiteralExpr*>(src))
            return located(std::make_unique<StringLiteralExpr>(expr->value), src);
        if (auto* expr = dynamic_cast<const BoolLiteralExpr*>(src))
            return located(std::make_unique<BoolLiteralExpr>(expr->value), src);
        ...
        if (auto* expr = dynamic_cast<const CallExpr*>(src)) {
            ...
            clone->iteratorRecipeStateName = expr->iteratorRecipeStateName;
            clone->iteratorRecipeSourceType = substitute(expr->iteratorRecipeSourceType);
            clone->iteratorCollectTargetType = substitute(expr->iteratorCollectTargetType);
            clone->iteratorCollectBuilderType = substitute(expr->iteratorCollectBuilderType);
            clone->iteratorCollectBeginSymbol = expr->iteratorCollectBeginSymbol;
            clone->iteratorCollectPushSymbol = expr->iteratorCollectPushSymbol;
            clone->iteratorCollectFinishSymbol = expr->iteratorCollectFinishSymbol;
            clone->compileTimeValue = expr->compileTimeValue;
            clone->compileTimeDeclarationId = expr->compileTimeDeclarationId;
            clone->isCompileTimeSymbolSet = expr->isCompileTimeSymbolSet;
            clone->compileTimeSymbolSetDeclarationIds = expr->compileTimeSymbolSetDeclarationIds;
            clone->isCompileTimeQueryDeclarationView = expr->isCompileTimeQueryDeclarationView;
            clone->compileTimeDeclarationViewDeclarationIds =
                expr->compileTimeDeclarationViewDeclarationIds;
            clone->isCompileTimeQueryDeclarationRef = expr->isCompileTimeQueryDeclarationRef;
            clone->isCompileTimeOptionalDeclarationRef = expr->isCompileTimeOptionalDeclarationRef;
            clone->compileTimeOptionalHasValue = expr->compileTimeOptionalHasValue;
            return located(std::move(clone), src);
        }
        ...
        if (auto* expr = dynamic_cast<const HeapAllocExpr*>(src)) {
            ...
            clone->allocatedTypeAST = cloneType(expr->allocatedTypeAST.get());
        ...
        if (auto* expr = dynamic_cast<const TryExpr*>(src)) {
            ...
            clone->operand = cloneExpr(expr->operand.get());
            clone->resultType = substitute(expr->resultType);
            clone->propagatedResultType = substitute(expr->propagatedResultType);
            clone->valueType = substitute(expr->valueType);
            clone->errorType = substitute(expr->errorType);
            clone->propagatedErrorType = substitute(expr->propagatedErrorType);
            clone->errorConversionSymbol = expr->errorConversionSymbol;
        ...
        if (auto* expr = dynamic_cast<const BlockExpr*>(src)) {
            ...
            return located(std::make_unique<BlockExpr>(std::move(block)), src);
        }
        ...
        if (auto* expr = dynamic_cast<const SelectExpr*>(src)) {
            ...
            clone->resolvedDeclarationId = expr->resolvedDeclarationId;
            clone->resolvedSymbolId = expr->resolvedSymbolId;
            clone->resolvedContractId = expr->resolvedContractId;
            clone->resolvedSymbolName = expr->resolvedSymbolName;
            clone->resolvedFamilyId = expr->resolvedFamilyId;
            clone->resolvedSelectorDeclarationId = expr->resolvedSelectorDeclarationId;
    ...
    std::unique_ptr<Stmt> cloneStmt(const Stmt* src) {
        if (!src) return nullptr;
        if (auto* statement = dynamic_cast<const BlockStmt*>(src)) return cloneBlock(statement);
        if (auto* statement = dynamic_cast<const LetStmt*>(src)) {
            ...
            clone->typeAnnotation = cloneType(statement->typeAnnotation.get());
            clone->initializer = cloneExpr(statement->initializer.get());
            clone->inferredType = substitute(statement->inferredType);
            clone->materializesIteratorRecipe = statement->materializesIteratorRecipe;
            clone->materializedIteratorOwnsSource = statement->materializedIteratorOwnsSource;
        ...
        if (auto* statement = dynamic_cast<const MatchStmt*>(src)) {
            ...
            clone->isCompileTimeOptionalMatch = statement->isCompileTimeOptionalMatch;
            clone->compileTimeSelectedArm = statement->compileTimeSelectedArm;
            for (const auto& sourceArm : statement->arms) {
                ...
                arm.bindingUsageDefaults = sourceArm.bindingUsageDefaults;
        ...
        if (auto* statement = dynamic_cast<const ForStmt*>(src)) {
            ...
            clone->isCompileTimeDeclarationViewLoop = statement->isCompileTimeDeclarationViewLoop;
            clone->compileTimeDeclarationIds = statement->compileTimeDeclarationIds;
            clone->compileTimeSymbolNames = statement->compileTimeSymbolNames;
            clone->compileTimeSymbolIds = statement->compileTimeSymbolIds;
            clone->compileTimeContractIds = statement->compileTimeContractIds;
            clone->protocolNextSymbol = statement->protocolNextSymbol;
            clone->protocolIteratorType = substitute(statement->protocolIteratorType);
            clone->protocolOptionType = substitute(statement->protocolOptionType);
            clone->protocolNoneVariant = statement->protocolNoneVariant;
            clone->protocolSomeVariant = statement->protocolSomeVariant;
            clone->protocolIntoSymbol = statement->protocolIntoSymbol;
            clone->protocolInputType = substitute(statement->protocolInputType);
            clone->protocolStateName = statement->protocolStateName;
            clone->protocolStateNeedsCleanup = statement->protocolStateNeedsCleanup;
            clone->protocolStateCleanup = statement->protocolStateCleanup;
            clone->recipeStateName = statement->recipeStateName;
            clone->recipeSourceType = substitute(statement->recipeSourceType);
            return located(std::move(clone), src);
        }
        ...
        if (auto* statement = dynamic_cast<const SlotInvokeStmt*>(src)) {
            ...
            clone->isImplicitCapture = statement->isImplicitCapture;
...
FunctionDecl* TypeResolver::monomorphize(FunctionDecl* generic, const TypeVec& concreteTypes) {
    ...
    request.genericDeclarationId = functionDeclarationIdentity(mContext.mProgram, generic);
    ...
    request.requestedBy = mContext.mDiagnosticFile + ":" +
                          std::to_string(mContext.mDiagnosticLine) + ":" +
                          std::to_string(mContext.mDiagnosticCol);
    const std::string requestKey = luna::instantiation::Instantiator::keyFor(request);
    ...
    for (auto& p : generic->params) {
        ...
        TypePtr parameterType =
            p.inferredType
                ? substituteNominalType(mContext.mConstraints.resolve(p.inferredType), bindings)
                : (p.type ? resolveType(p.type.get(), bindings) : TyUnknown);
    ...
    if (generic->returnType) {
        const TypePtr returnType =
            generic->inferredReturnType
                ? substituteNominalType(mContext.mConstraints.resolve(generic->inferredReturnType),
                                        bindings)
        ...
        if (specialized->returnsLinear)
            specialized->returnType =
                std::make_unique<LinearTypeAST>(std::move(specialized->returnType));
        else if (generic->returnUsage == luna::ownership::Usage::Affine)
            specialized->returnType =
                std::make_unique<AffineTypeAST>(std::move(specialized->returnType));
    }
