...
Default implementation of Luna runtime C ABI entry points, covering host-service installation, memory management (plain/RC/ARC), console/file I/O, dynamic GPU backend loading (CUDA/ROCm), and error snapshots.
...
The master declaration header of the Luna runtime's C ABI entry points, covering host environment installation, memory management, console/file I/O, GPU backend management, error snapshots, and array bounds checking.
...
- Declares the internal v1 standard-library console adapters (five functions such as `rt_console_write_cstr_v1`).
...
### Console I/O and library adapters
...
| `rt_console_write_cstr_v1` and others | Initial 0.3 cstr/i32 standard-library adapter over the raw host console ABI |
