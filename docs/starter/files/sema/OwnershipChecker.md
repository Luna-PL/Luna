...
- `checkSlotInvoke(slot)`: resolves the innermost static apply or the default
  fragment. For Many (multi-shot), it snapshots and checks the continuation
  body once; if captured state is consumed (`continuationConsumesCapturedState`),
  the fragment cannot resume it multiple times.
