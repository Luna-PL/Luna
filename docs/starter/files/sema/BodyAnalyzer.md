...
- `analyzeCall`/`analyzeMemberCall`/`analyzeIteratorCall`/`analyzeIntrinsicCall`: resolution and type constraints for function calls, member methods (trait impl methods), iterators (protocol/recipe), and builtins/intrinsics.
