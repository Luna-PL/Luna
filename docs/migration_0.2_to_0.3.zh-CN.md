...
| Rc/Arc | 编译器特判 `rc new T(...)`、`arc new T(...)` 与专用 TypeKind | 实现 Resource/Drop 的普通名义 `Rc::new(value)`/`Arc::new(value)` 容器 | 删除跨编译器特判，让库负责容器策略 | 显式构造库容器并显式 clone handle；不提供兼容 desugaring |
...
| MoonIR | pointer-heavy 内存 IR；`--emit-moonir` 是诊断输出 | 内存/序列化共用的单一 canonical table-referenced MoonIR | Moon Container 需要稳定 identity 与验证，且不应引入第二个 IR | tooling 迁移到 canonical model；wire 细节见 `TBD-M005` |
