...
| 阶段/retention | CompileTime、Runtime、Dynamic retention；`dynamic select`、`dynamic slot`、`dynamic apply` | 仅 compile-time/runtime；运行时行为由 value、descriptor、query、builtin 与 EV004 宿主 binding 表达 | Dynamic 混合 retention、discovery 与 dispatch | 所有 Dynamic 源码 form 均拒绝；改用 compile-time Symbol Query、静态 Slot composition 或显式 typed host binding |
...
| Slot/Fragment | 函数内 slot statement、结构 contract、`apply name(fragment)` 和动态有限候选 | 模块级 `slot kind name(params) [default fragment];`、`kind fragment(params) for slot { ... }`、词法 `slot(args) { ... }` 与 `apply fragment { ... }` | local string 与结构 shape 无法支撑安全开放扩展 | 改写为 SF006 名义 target；`many`、dynamic、局部 slot 与无 body apply 均被拒绝 |
...
| 进化 | 没有 Moon Container generation loop | host-only C++17 `Evolution.h` staging、验证、安全点 activation、pinned/switchable binding 与 rollback | 进化必须显式、受验证且不进入普通调用热路径 | 嵌入 EV004 宿主 API；没有 Luna 源码/CLI activation 拼写，且 0.3 不迁移持久状态 |

任何一行都不授权在 0.3 编译器中加入兼容代码。已删除语法必须迁移到表中所列 0.3
目标，或继续使用已冻结的 0.2 编译器。
