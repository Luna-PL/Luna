...
Luna 目前提供显式的 C ABI 边界。它仅用于用户明确声明的外部库；
语言内部的分配、清理与输出已迁移到 [Runtime ABI](runtime_abi.md)，不会通过
原始 C 符号隐式完成。导入使用 `extern "C"`；若目标符号名和 Luna 名不同，
使用 `as` 指定链接名：
...
`new` 的产物，不能使用 Luna `free`/`rt_dealloc`。目前由声明者维护这个配对；
Runtime ABI 已预留 `LunaOwnedForeignMemoryV1` 的 release capability，后续的类型化
adapter 会把这个匹配从约定提升为可验证合约。Luna 不会推测外部函数是否保存
借用、释放指针或异步访问内存，因此此边界由声明者负责精确表达。
