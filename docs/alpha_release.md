# Luna Alpha 0.1.0 发布说明

## 定位

Luna Alpha 0.1.0 是用于语言试验和内部项目的 Linux/macOS/Windows 编译器原型。三个平台的稳定核心均由 CI 工作流验证；Windows 使用 MSYS2 UCRT64 工具链。稳定核心包括多文件包与显式导出、基础函数/泛型/ADT、线性所有权与借用、C FFI、JIT/AOT、`-O0/-O2/-O3`，以及 CPU 模拟器上的初始异构 ABI。
...
- ROCm 冒烟测试需要可见 AMD GPU、HIP 运行时、内核驱动和匹配的 `LUNA_AMDGPU_ARCH`。基础 JIT/AOT 冒烟已在 RX 7800 XT 上通过；更广泛的 GPU 型号、长时间稳定性与性能数据仍待积累。
