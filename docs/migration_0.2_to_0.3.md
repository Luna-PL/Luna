...
| Phases/retention | CompileTime, Runtime, and Dynamic retention; `dynamic select`, `dynamic slot`, and `dynamic apply` | compile-time/runtime only; runtime behavior uses values, descriptors, queries, builtins, and EV004 host bindings | Dynamic mixes retention, discovery, and dispatch | all Dynamic source forms are rejected; use compile-time Symbol Query/static Slot composition or an explicit typed host binding |
...
| Rc/Arc | compiler-special `rc new T(...)`, `arc new T(...)`, and special TypeKinds | implemented ordinary nominal Core `Rc::new(value)`/`Arc::new(value)` containers | removes cross-compiler special cases and lets libraries own container policy | import `org.luna.core`, construct explicitly, and duplicate handles through `Clone`; 0.3 directly rejects the old spelling |
| Layout reflection | `type_size::<NamedStruct>()` summed source-field sizes despite the actual pointer value ABI | `type_size`/`type_alignment` report the real value-slot ABI | generic containers must allocate and load with the actual size/alignment | do not use `type_size` to estimate named-product field storage; a current 64-bit pointer-represented struct value is 8/8 |
| Slot/Fragment | function-local slot statements, structural contracts, `apply name(fragment)` and dynamic finite candidates | module-level `slot kind name(params) [default fragment];`, `kind fragment(params) for slot { ... }`, lexical `slot(args) { ... }`, and `apply fragment { ... }` | local strings and structural shapes cannot support safe open-world extension | rewrite to the SF006 nominal target; `many`, dynamic, local-slot, and blockless-apply forms are rejected |
| MoonIR | pointer-heavy in-memory IR; `--emit-moonir` is a diagnostic output | one canonical table-referenced MoonIR with in-memory and serialized forms | Moon Containers require stable identity and verification without a second IR | framing, all eight payloads, the canonical CFG code codec, atomic verified loading, and a production JIT round trip are implemented |
...
| Evolution | no Moon Container generation loop | host-only C++17 `Evolution.h` staging, verification, safe-point activation, pinned/switchable binding, and rollback | evolution must be explicit, verified, and off ordinary call hot paths | embed the EV004 host API; there is no Luna source/CLI activation spelling and 0.3 does not migrate persistent state |

No row authorizes compatibility code in the 0.3 compiler. Removed syntax must be migrated to
the stated 0.3 destination or kept on the frozen 0.2 compiler.
